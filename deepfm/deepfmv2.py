...
class DeepFmModel():
	def __init__(self,is_training,batch_size,feature_size,class_num,deep_layers=[256,128,64],dropout_keep_deep=[1,1,1,1],dropout_keep_fm=[1,1]):
		...
		# 损失函数的定义：均方差
			...
			score1 = tf.round(score)
			score1=tf.cast(score1, tf.int32)
			self.accuracy  = tf.contrib.metrics.accuracy(score1, y)
			self.auc = tf.contrib.metrics.streaming_auc(score,tf.convert_to_tensor(self.y))   
			##观看常量
