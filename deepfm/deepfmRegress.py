...
class DeepFmModel():
	def __init__(self,is_training,batch_size,feature_size,class_num,isregress=False):
		'''fm部分 '''
		...
		if not isregress:
			...
			self.y = tf.placeholder(shape=[batch_size,1],name='yout',dtype=tf.float32)
		...
		if not isregress:
			...
			with tf.variable_scope('deepfm'):
				print(left.get_shape(),right.get_shape(),ydeep.get_shape())
				deepfm = tf.concat([left,right,ydeep],axis=1)
				deepfm	= tf.contrib.layers.fully_connected(deepfm, 1, \
				activation_fn=None, scope = 'deepfm_regress')
				
				
			with tf.variable_scope('loss'):
				self.loss = tf.sqrt(tf.reduce_mean(tf.reduce_sum(tf.square(self.y - deepfm),reduction_indices=[1])))
				self.train_op = tf.no_op
				if is_training:
					self.train_op=tf.train.AdamOptimizer(learning_rate=0.001, beta1=0.9, beta2=0.999,epsilon=1e-8).minimize(self.loss)
			#	correct_prediction = tf.equal(tf.argmax(deepfm,1), tf.argmax(self.y,1)) # 计算准确度
...
def runEpoch(session,model,xdata,ydata,epoch_size):
	...
	for step in range(epoch_size):
		...
		feed_dict[model.y]=y
		loss,_=session.run([model.loss,model.train_op],feed_dict=feed_dict)
		totalloss += loss
		#totalacc += acc
	...
	print('run finish,avgloss=%f'%(avgloss))
	return avgloss
...
with tf.variable_scope('trainfm'):
	trainmodel = DeepFmModel(True,batch_size,feature_size,200,isregress=True)
	
with tf.variable_scope('testfm'):
	testmodel = DeepFmModel(False,batch_size,feature_size,200,isregress=True)
